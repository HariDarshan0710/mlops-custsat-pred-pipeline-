...
from run_deployment import run_deployment  # Import the correct function

def run_main():
    """
    Function to run the deployment pipeline, which should trigger the
    training and deployment of the model if it's not already available.
    """
    try:
        # Call the main function from run_deployment.py
        run_deployment()
        st.success("The pipeline has been triggered and the model is being deployed.")
    except Exception as e:
        st.error(f"An error occurred while running the pipeline: {e}")

def mainrun_deployment():
    ...
    if st.button("Results"):
        ...
        st.write(
            "Following figure shows how important each feature is in the model that contributes to the target variable or contributes to predicting the customer satisfaction rate."
        )
